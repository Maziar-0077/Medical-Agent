...
def nlu_extraction_node(state):
    ...
    system_prompt = (
        ...
        "### IMPORTANT: EXTRACT FULL NAME\n"
        "When extracting the patient's name, capture the full name (first and last name) as provided by the patient (e.g., 'John Smith', 'Sara Bahrami'). Do not truncate to a single word.\n\n"
    ...
    field_keywords = {
        ...
        'medications': ['medication', 'medications', 'drugs', 'current meds', 'taking any medications']
    }
...
def derailment_detector_node(state):
    ...
    # Use a model that reliably handles structured output for derailment detection
    derailment_llm = ChatGroq(
        model="llama-3.3-70b-versatile",  # This model works well with tool calls
...
def planner_node(state):
    ...
    if is_new_patient:
        ...
        # For returning patients, we must ask about correlation with previous visit
        checklist = [
            ("symptoms", "symptoms"),
            ("duration", "duration"),
            ("pain_severity", "pain severity"),
...
def generate_final_response_node(state):
    ...
    # Include previous visit correlation in the final output if available
    correlation_summary = ""
    ...
    return {"final_output": msg, "messages": [AIMessage(content=msg)]}# nodes.py
from langchain_core.messages import AIMessage, HumanMessage, SystemMessage
from config import main_llm, auditor_llm, referral_llm, embedding_llm, extraction_llm
from schemas import (
    PatientInformation, ExtractedDetails, DynamicPlannerDecision,
